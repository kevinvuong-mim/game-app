...
import { MAP_COUNT, getMapDefinition, mapBackgroundKey } from '@game/campaign/mapConfig';
import {
  getLastMapId,
  getLevelStars,
  isLevelPlayable,
...
export class MapScene extends Phaser.Scene {
  ...
  private renderMap(): void {
    ...
    const viewportH = (VISIBLE_ROWS - 1) * ROW_STRIDE + BOX_SIZE;
    ...
    const navY = gridY + viewportH + PANEL_PAD + 94;
    const prev = createUIButton({
      scene: this,
      position: { x: width * 0.28, y: navY },
      size: { width: 200, height: 84 },
      background: { key: 'settings-button-background' },
      disabled: this.mapId <= 1,
      text: {
        content: t('map.previous'),
        style: { fontSize: 28, fontStyle: 'bold', border: { width: 4, color: '#000000' } },
      },
      onClick: () => this.changeMap(-1),
    });
    const next = createUIButton({
      scene: this,
      position: { x: width * 0.72, y: navY },
      size: { width: 200, height: 84 },
      background: { key: 'settings-button-background' },
      disabled: this.mapId >= MAP_COUNT,
      text: {
        content: t('map.next'),
        style: { fontSize: 28, fontStyle: 'bold', border: { width: 4, color: '#000000' } },
      },
  ...
  private bindGridScroll(): void {
    ...
    const hit = this.add
      .rectangle(x + width / 2, y + height / 2, width + PANEL_PAD * 2, height + PANEL_PAD * 2, 0x000000, 0)
      .setInteractive()
      .setDepth(4);
  ...
  private addLevelBox(
    ...
    const label = this.add
      .text(x, y - 18, String(levelIndex + 1), {
  ...
  private changeMap(delta: number): void {
    const nextId = this.mapId + delta;
    if (nextId < 1 || nextId > MAP_COUNT) return;
    this.mapId = nextId;
    this.renderMap();
  }
