...
Product IDs trong `src/platform/modules/iap/iap.config.ts` (`remove_ads`, `coins_10000`) phải khớp App Store Connect / Play Console / RevenueCat. UI lấy giá localized qua `iap.getDisplayPrice` (store `priceString`; fallback `$0.98` / `$3.98`). Storefront Apple thường trả `US$…` cho USD — client normalize bỏ prefix `XX$`.
