...
Games talk to the platform primarily via the **Event Bus**. `@platform/ui` exports Phaser UI (panels, toast, i18n `t`, `BasePanelScene`) and re-exports the module services the game layer needs (`gameSync`, `shareService`, `rateService`, `gameRunService`, `shop`, `ads`, `missions`, `dailyRewards`, `navigationService`). ESLint blocks `@platform/modules/*` and several `@platform/core/*` imports from `src/game`.
...
**i18n / toast / share / rate / game-sync:** import from `@platform/ui` in the game layer.
...
| ads (module)  | Local    | Placement config (`HOME` / `SHOP` / `LEADERBOARD` banner, `MISSION_WATCH` + `DOUBLE_COINS` rewarded, `GAME_OVER` interstitial); `adsRemoved` chặn cả rewarded |
...
Built-in user-facing scenes: Home, Gameplay, GameOver, Shop, Missions, Leaderboard, DailyReward, Settings, HowToPlay, Legal.
